...
for (i in 1:6) {
  df$Activity_number[df$Activity == i] <-activity_lables$V2[i]
}
...
df2<-summarise_each(grouped_data,mean)
df2<-arrange(df2,Subject_number)

# Uploading table

write.table(df2,file = "C:/Users/rudolpa/OneDrive - Jacobs/R Training/Cleaning Data Course/getdata_projectfiles_UCI HAR Dataset/UCI HAR Dataset/final_table.txt",row.names = FALSE)